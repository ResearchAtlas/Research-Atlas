...
- [~] Codex CLI local preflight — **PARTIAL** (retry ready) — see [`preflight-codex.md`](preflight-codex.md). First attempt (2026-04-19) did not follow the protocol: corpus was pasted without the `verify these references - detailed depth, markdown output` prefix, so the skill's auto-trigger did not fire and Codex did a manual imperative reconstruction via Playwright against CrossRef + OpenAlex instead of running the skill. No `schema_version: 2` envelope was emitted, so the L2 pass conditions cannot be evaluated against a skill run. Codex self-admitted the miss in its closing statement. Parity sub-evidence (non-binding) was consistent with Claude's L1 (21 verified / 4 unverifiable / 5 traps flagged; +1 delta on ref 19 Bengio NPLM within the ±1 tolerance). Two L2-specific findings were triaged: **Finding A** — Codex's `/skills` panel listed `research-verification` twice. Root cause (confirmed via P1 review): Codex's native discovery walks both `.codex/skills/` and `.agents/skills/` as sibling workspace roots; `.codex/skills/research-verification/` was a stale v2.0.0 copy, `.agents/skills/research-verification/` is the mirrored v2.1.0. Running L2 while the duplicate was live risked executing against stale v2.0.0 instructions. **Fixed locally** by removing `.codex/skills/research-verification/` (the `.codex/` tree is gitignored, so no commit needed); the other nine legacy v2.0.0 skills under `.codex/skills/` are non-blocking for L2 and stay as a post-v1 cleanup. **Finding B** — auto-match on description phrases did not fire; retry must either prefix the trigger per protocol step 3 or invoke `$research-verification` explicitly per RUN-COMMANDS.md. Retry protocol captured in the transcript.
